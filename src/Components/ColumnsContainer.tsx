...
export default function ColumnContainer(props: Props) {
    ...
    if (isDragging) {
        return <div ref={setNodeRef} style={style} className="h-[500px] border-rose-600 opacity-60 w-[350px] min-h-[500px] cursor-pointer rounded-lg bg-mainBackgoundColor border-2 flex flex-col"></div>;
    }
            ...
            <div
                ...
                <div
                    ...
                    <div className="flex gap-2">
                        <div className="flex rounded-full justify-center items-center bg-slate-700 px-2 py-1 text-sm">
                            {column.tasks.length} {/* Menampilkan jumlah tugas */}
                        </div>
                ...
                <div className="flex flex-grow flex-col gap-4 p-2 overflow-x-hidden overflow-y-auto">
                    <SortableContext items={tasksIds}>
                        {tasks.map(task => (
                            <TaskContainer key={task.id} tasks={task} />
                        ))}
                    </SortableContext>
                </div>
            </div>
