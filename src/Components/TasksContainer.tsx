...
export default function TaskContainer(props: Props) {
    ...
    const getBorderColor = () => {
        const colors = ["border-pink-300", "border-blue-300", "border-yellow-200", "border-lime-300"];
        return colors[(columnId - 1) % colors.length]; // Gunakan columnId
    };

    const getDragColor = () => {
        const colors = ["border-pink-400", "border-blue-400", "border-yellow-200", "border-lime-400"];
        return colors[(columnId - 1) % colors.length]; // Gunakan columnId
    };

    if (isTaskDragging) {
        return <div
            className={`${getDragColor()} opacity-50 mx-4 bg-white p-2 h-[45px] min-h-[45px] flex justify-center items-center text-center rounded-lg border-2 cursor-grab relative`}
            ref={setTaskRef}
            style={style}
        />
    }
    ...
    return (
        <>
            <div
                ...
                className={`bg-white  ${userTask ? `border-2 ${getBorderColor()}` : `border border-slate-500`}  -mx-2  p-2.5 h-[50px] min-h-[50px] flex items-center text-center justify-center rounded-lg shadow-md cursor-pointer ${userTask ? `hover:bg-[#b7ddfd]` : `hover:bg-[#ff949b]`} `}>
                <p className="text-gray-700 text-sm overflow-hidden whitespace-pre-wrap ">
                    {tasks.title}
                </p>
            </div >
            ...
            <Modal show={openModal} onClose={() => setOpenModal(false)}>
                ...
                <Modal.Footer className="flex justify-between">
                    <button
                        ...
                        onClick={() => deleteTask(columnId, tasks.id)} color="red">
                        Delete Task
                    </button>
                    <button
